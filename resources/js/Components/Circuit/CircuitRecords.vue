...
<template>
    <div>
        <h2 class="mb-3 text-lg font-bold text-white">Рекорди на пистата</h2>

        <div class="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            <div v-for="card in countCards" :key="card.label" class="rounded-xl border border-zinc-800 bg-zinc-900/60 p-4">
                <div class="text-xs uppercase tracking-wide text-zinc-500">{{ card.icon }} {{ card.label }}</div>
                ...
                <div v-else class="mt-1 text-sm text-zinc-600">—</div>
            </div>
        </div>

        <div class="mt-3 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            <div class="rounded-xl border border-zinc-800 bg-zinc-900/60 p-4">
                <div class="text-xs uppercase tracking-wide text-zinc-500">🥇 Първи победител</div>
            ...
            <div class="rounded-xl border border-zinc-800 bg-zinc-900/60 p-4">
                <div class="text-xs uppercase tracking-wide text-zinc-500">🆕 Последен победител</div>
            ...
            <div class="rounded-xl border border-zinc-800 bg-zinc-900/60 p-4">
                <div class="text-xs uppercase tracking-wide text-zinc-500">🎯 Pole → победа</div>
                <div v-if="poleConv !== null && poleConv !== undefined" class="mt-1">
                    <div class="text-2xl font-black tabular-nums text-white">{{ poleConv }}%</div>
            ...
            <div class="rounded-xl border border-zinc-800 bg-zinc-900/60 p-4">
                <div class="text-xs uppercase tracking-wide text-zinc-500">📊 Средна стартова поз. на победителя</div>
                <div v-if="avgGrid !== null && avgGrid !== undefined" class="mt-1">
                    <div class="text-2xl font-black tabular-nums text-white">P{{ avgGrid }}</div>
                </div>
                <div v-else class="mt-1 text-sm text-zinc-600">—</div>
            </div>
        </div>
