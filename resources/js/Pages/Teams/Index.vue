    ...
    <PublicLayout>
        ...
        <div class="mb-5 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            <input
                ...
                aria-label="Търсене на отбор"
                class="w-full rounded-lg border border-zinc-800 bg-zinc-900/60 px-4 py-2 text-sm text-white placeholder-zinc-500 focus:border-red-600 focus:outline-none focus:ring-1 focus:ring-red-600 sm:w-64"
            />
        </div>
        ...
        <div v-else class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            ...
            >
                <div class="absolute inset-y-0 left-0 w-1" :style="{ backgroundColor: team.color_hex ?? TEAM_COLOR_FALLBACK }" />
                <div class="flex items-center gap-4">
                    ...
                    <div class="min-w-0 flex-1">
                        <div class="flex items-center gap-2">
                            <span class="truncate font-display text-lg font-bold text-white">{{ team.name_bg ?? team.name }}</span>
