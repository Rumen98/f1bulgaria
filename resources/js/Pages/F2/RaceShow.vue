    ...
    <PublicLayout>
        ...
        <section class="mt-3 rounded-2xl border border-zinc-800 bg-gradient-to-br from-zinc-900 to-black p-6">
            <div class="flex flex-wrap items-center gap-3">
                ...
                <h1 class="text-2xl font-black sm:text-3xl">{{ race.location }} <span class="text-zinc-500">{{ race.season }}</span></h1>
            ...
            <div class="mt-3 flex flex-wrap gap-2 text-sm">
                <Link v-if="hasSprint" :href="route('f2.race', [race.slug, 'sprint'])"
                    class="rounded-lg px-3 py-1.5 font-medium transition" :class="sessionType === 'sprint' ? 'bg-sky-600 text-white' : 'bg-zinc-900 text-zinc-400 hover:text-white'">Спринт</Link>
                <Link v-if="hasFeature" :href="route('f2.race', [race.slug, 'feature'])"
                    class="rounded-lg px-3 py-1.5 font-medium transition" :class="sessionType === 'feature' ? 'bg-red-600 text-white' : 'bg-zinc-900 text-zinc-400 hover:text-white'">Главно</Link>
                <Link v-if="race.circuit_jolpica_id && has('circuits.show')" :href="route('circuits.show', race.circuit_jolpica_id)"
                    class="rounded-lg bg-zinc-900 px-3 py-1.5 font-medium text-zinc-400 transition hover:text-white">Пистата →</Link>
            </div>

            <div class="mt-4 flex flex-wrap gap-3">
                <div v-if="pole" class="rounded-xl border border-zinc-800 bg-zinc-950/60 px-4 py-2 text-sm">
                    <span class="text-zinc-500">Pole:</span> <span class="font-semibold text-white">{{ pole.flag }} {{ pole.driver }}</span>
                </div>
                <div v-if="fastestLap" class="rounded-xl border border-zinc-800 bg-zinc-950/60 px-4 py-2 text-sm">
                    <span class="text-zinc-500">⏱️ Най-бърза:</span> <span class="font-semibold text-white">{{ fastestLap.flag }} {{ fastestLap.driver }}</span>
        ...
        <!-- Results -->
        <div class="mt-6 overflow-x-auto rounded-xl border border-zinc-800">
            <table class="w-full text-sm">
                <thead class="bg-zinc-900/80 text-xs uppercase tracking-wide text-zinc-500">
                    <tr>
                        <th class="px-3 py-2 text-left">Поз.</th>
                        <th class="px-2 py-2 text-left">#</th>
                        <th class="px-3 py-2 text-left">Пилот</th>
                        <th class="hidden px-3 py-2 text-left sm:table-cell">Отбор</th>
                        <th class="hidden px-2 py-2 text-right md:table-cell">Старт</th>
                        <th class="px-3 py-2 text-right">Време/Изоставане</th>
                        <th class="px-2 py-2 text-right">Точки</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-zinc-800/60">
                    <tr v-for="r in results" :key="r.slug + r.position" :class="[rowClass(r.position), r.is_bulgarian ? 'ring-1 ring-inset ring-emerald-500/40' : '']">
                        ...
                        <td class="px-3 py-2.5 font-semibold text-white">
                            <Link v-if="r.slug && has('f2.drivers.show')" :href="route('f2.drivers.show', r.slug)" class="transition hover:text-red-400">
                                {{ r.flag }} {{ r.driver }}
                            </Link>
                            <span v-else>{{ r.flag }} {{ r.driver }}</span>
                            <span v-if="r.fastest_lap" title="Най-бърза обиколка" class="ml-1 text-purple-400">⏱️</span>
                        </td>
