...
export class EngineSound {
    ...
    update(rpm, throttle, maxRpm) {
        ...
        if (this.usingSample) {
            // ПОЧТИ без изкуствен питч (само лек нюанс): реалният запис сам си
            // надува и сменя предавки — това е истинският звук. Агресивното пичане
            // на почти постоянен тон звучеше като „забила предавка".
            this.source.playbackRate.setTargetAtTime(0.96 + rev * 0.14, now, 0.08);
            // Силата зависи ПРЕДИМНО от газта — почти тихо при отпускане, силно при
            // ускорение. Така реалната V10 се „наслагва" на газта/ускорението, а не
            // върви постоянно като пуснат клип.
            const target = 0.03 + throttle * 0.8 + rev * 0.05;
            this.master.gain.setTargetAtTime(Math.min(1, target), now, 0.05);
            return;
        }
