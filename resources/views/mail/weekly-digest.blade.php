...
{{--
    Човек без нито една прогноза не бива да получава таблица от нули — „точки 0, прогнози 0, най-добър резултат 0“ е демотивиращо и заема мястото на единственото полезно нещо за него: поканата да започне. --}}
...
@if ($quiz)
## Куизът на Падок 🧠
Имаш **{{ $quiz['points'] }}** {{ $quiz['points'] == 1 ? 'точка' : 'точки' }}. Новите въпроси излизат всеки понеделник — [към куиза]({{ url('/quiz') }}).
@endif
...
{{--
    Приоритетът на бутона следва това, което липсва на получателя: непознат има нужда от акаунт, играч без прогноза за следващия кръг — от прогноза, и чак редовният играч печели от класирането. Куизът нарочно е само инлайн линк по-горе — втори бутон би размил тази подредба. --}}
@if (! $userStats)
...
До следващото състезание! 🏁<br> Екипът на Падок
