...
it('грешните форми се срещат само като изричен контрапример', function (string $wrong) {
    ...
    while (($pos = mb_strpos($prompts, $wrong, $offset)) !== false) {
        // Отрицанието е ту с главни („НЕ „Гран При""), ту с малки букви
        // („(не „Гран При")") — проверката е нечувствителна към регистъра.
        $before = mb_substr($prompts, max(0, $pos - 30), min(30, $pos));

        expect(mb_stripos($before, 'не') !== false)
            ->toBeTrue("„{$wrong}“ се среща в промпта без отрицание пред него: „…{$before}“");
