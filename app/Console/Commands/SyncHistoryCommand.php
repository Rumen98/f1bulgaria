 ...
 * продължава със следващите и докладва обобщение накрая.
 */
class SyncHistoryCommand extends Command
{
    ...
    public function handle(SeasonSyncService $seasons, ResultSyncService $results): int
    {
        ...
        foreach (range($from, $to) as $year) {
            ...
            [$ok, $failed] = $this->syncSeasonResults($year, $results, $throttleMs);
     ...
     */
    private function syncSeasonResults(int $year, ResultSyncService $results, int $throttleMs): array
    {
        $races = Race::query()
