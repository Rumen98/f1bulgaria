...
/**
 * Постовете на играта в канала: старт на седмичното предизвикателство
...
class GameChallengeFormatter
{
    ...
    public function challenge(string $trackName, string $url): string
    {
        return implode("\n", [
            ...
            'Запиши обиколка в играта и влез в седмичната класация.',
            '',
    ...
    public function results(string $trackName, string $slug, Collection $top, string $gameUrl): string
    {
        $medals = ['🥇', '🥈', '🥉'];
        $lines = [
            '🏆 <b>Играта: резултати от '.TelegramText::escape($trackName).'</b>',
            '',
        ];
